...
state = ""
"""
    This bubble controls the state of the program so that after
    finishing the user can decide if they want to try it again.
"""
while state == "y" or state == "":
    print("\n----        Program palindrome        ----\n")
    ...
    # Convert the string to a list and a reverse
    # copy to loop through them in time
    string = list(s)
    changes = 0
    left = 0
    right = len(string) - 1
    ...
    # Ask and control the status of the program
    state = str(input("Do you want to try again? y/n: "))
    # Controls whether the state is y or n
    while state != "n" and state != "y":
        state = str(input("You must answer y or n: "))
